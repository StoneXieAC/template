...
std::vector<Point> calHalfPlaneIntersection(std::vector<Seg> e) { // 默认取左侧的半平面
    std::sort(e.begin(), e.end(), [&](const Seg &a, const Seg &b) {
        auto ang1 = atan2((a.p2 - a.p1).y, (a.p2 - a.p1).x);
        auto ang2 = atan2((b.p2 - b.p1).y, (b.p2 - b.p1).x);
        if (dcmp(ang1, ang2) == 0) return sign((a.p2 - a.p1) % (b.p1 - a.p1)) < 0;
        return dcmp(ang1, ang2) < 0;
    });
    e.resize(
        std::unique(e.begin(), e.end(), [&](const Seg &a, const Seg &b) {
            return dcmp(atan2((a.p2 - a.p1).y, (a.p2 - a.p1).x),
                        atan2((b.p2 - b.p1).y, (b.p2 - b.p1).x)) == 0;
        }) - e.begin());
    auto check = [&](const Seg &a, const Seg &b, const Seg &c) {
        auto t = calSegCross(b, c);
        return sign((t - a.p2) % (a.p1 - a.p2)) < 0;
    };
    std::deque dq{e[0], e[1]};
    for (int i = 2; i < e.size(); i++) {
        while (dq.size() > 1 && check(e[i], dq.end()[-1], dq.end()[-2]))dq.pop_back();
        while (dq.size() > 1 && check(e[i], dq.begin()[0], dq.begin()[1]))dq.pop_front();
        dq.push_back(e[i]);
    }
    while (dq.size() > 1 && check(dq.begin()[0], dq.end()[-2], dq.end()[-1]))dq.pop_back();
    while (dq.size() > 1 && check(dq.end()[-1], dq.begin()[0], dq.begin()[1]))dq.pop_front();
    std::vector<Point> res;
    for (int i = 0; i + 1 < dq.size(); i++)
        res.push_back(calSegCross(dq[i], dq[i + 1]));
    if (dq.size() > 2) // dq.size() <= 2 交出来的不是多边形
        res.push_back(calSegCross(dq.front(), dq.back()));
    return res;
}